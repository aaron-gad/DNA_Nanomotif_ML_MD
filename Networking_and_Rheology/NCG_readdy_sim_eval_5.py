...
#####################
# Code adapted to use rotationally averaged, RPY tensor instead of Oseen tensor
#####################

def compute_pre_matrix_vec_rpy_reg(positions, box_size, eta=1e-3, a=5e-9):
    """
    Vectorized pre-averaged (isotropic) Rotne-Prager-Yamakawa mobility matrix.
    Reduces exactly to the isotropic Oseen tensor for r >= 2a, and switches
    to the regularised near-field RPY form for r < 2a (overlapping spheres),
    avoiding the divergence and potential loss of positive-definiteness of
    the plain Oseen tensor at short range.
    see also: https://pubs.acs.org/jctcce/article/19/15/5099/325706/Orientationally-Averaged-Version-of-the-Rotne
    positions: (frames, N, 3)
    """
    positions = np.asarray(positions)
    n_frames, N, _ = positions.shape
    H = np.zeros((N, N))
    prefactor_far = 1 / (6 * np.pi * eta)
    prefactor_near = 1 / (6 * np.pi * eta * a)

    # 1. Diagonal: self-mobility (Stokes-Einstein), same as before
    np.fill_diagonal(H, prefactor_far / a)

    # 2. Iterate over pairs
    for i in range(N):
        diff = positions[:, i:i+1, :] - positions[:, i+1:, :]
        diff -= box_size * np.round(diff / box_size)
        r = np.linalg.norm(diff, axis=2)  # (frames, N-(i+1))

        far_field = prefactor_far / r
        near_field = prefactor_near * (1.0 - r / (4.0 * a))

        # apply the piecewise RPY formula per frame, THEN average
        rpy_pair = np.where(r >= 2 * a, far_field, near_field)
        mean_rpy = np.mean(rpy_pair, axis=0)
...
#get averaged moduli from full graph laplacian
#this function takes get_time_avg_storage_loss_mod_1/2/3/4 and gives time and repeat averaged moduli
#version for RPY tensor
def avg_moduli_hydr_3_rpy(folder_names,omega_t_sim1,t_sim1,b_u,zeta_u,k_u,T_u,phi_u,time_step,time_skip,box_size,eta,a,t_r_avg_f,t_r_avg_l):
    storage_mod_all_sims=[]
    loss_mod_all_sims=[]
    #conn_mat_all_sims=[]
    cross_links_all_sims=[]
    for p in range(len(folder_names)):
        ...
        times_mech,types_mech,ids_mech,part_positions_mech=traj_mech.read_observable_particles()
        #storage_mod_avg_t_nc,storage_mod_stde_t_nc,loss_mod_avg_t_nc,loss_mod_stde_t_nc,relaxation_mod_avg_t_nc,relaxation_mod_stde_t_nc,conn_mat_all_t,eigv_all_t,tau_all_t=get_time_avg_storage_loss_mod(obs_top=obs_top_mech,time_step=time_step,time_skip=time_skip,omega_t=omega_t_sim1,t=t_sim1,edges_centre=4,edges_linker=3,cutoff_zero_eigv=10**-9,zeta=zeta_u,b=b_u,k=k_u,T=T_u,phi=phi_u)
        storage_mod_avg_t_nc,storage_mod_stde_t_nc,loss_mod_avg_t_nc,loss_mod_stde_t_nc,relaxation_mod_avg_t_nc,relaxation_mod_stde_t_nc,conn_mat_all_t,eigv_all_t,tau_all_t=get_time_avg_storage_loss_mod_hydr_rpy(obs_top=obs_top_mech,part_pos=part_positions_mech,time_step=time_step,time_skip=time_skip,omega_t=omega_t_sim1,t=t_sim1,edges_centre=4,edges_linker=3,cutoff_zero_eigv=10**-9,zeta=zeta_u,b=b_u,k=k_u,T=T_u,phi=phi_u,box_size=box_size,eta=eta,a=a,t_r_avg_f=t_r_avg_f,t_r_avg_l=t_r_avg_l)
        storage_mod_all_sims.append(storage_mod_avg_t_nc)
        loss_mod_all_sims.append(loss_mod_avg_t_nc)
...
#based on get_time_avg_storage_loss_mod, for hydr interaction
#time averaged storage and loss moduli
#Versin for RPY tensor
def get_time_avg_storage_loss_mod_hydr_rpy(obs_top,part_pos,time_step,time_skip,omega_t,t,edges_centre,edges_linker,cutoff_zero_eigv,zeta,b,k,T,phi,box_size,eta,a,t_r_avg_f,t_r_avg_l):
    storage_mod_all_t=[]
    loss_mod_all_t=[]
    relaxation_mod_all_t=[]
    conn_mat_all_t=[]
    ...
    for i in range(len(obs_top[:-t_r_avg_l][time_skip::time_step])):
        ...
        #print(" XXX time in get_time_avg_storage_loss_mod_hydr before get_conn_mat_eigv_hydro2 ",datetime.datetime.now().time())
        conn_mat_t,eigv_t=get_conn_mat_eigv_hydro2_rpy(time_sel=kth,obs_top=obs_top,part_pos=part_pos,edges_centre=edges_centre,edges_linker=edges_linker,box_size=box_size, eta=eta, a=a,t_r_avg_f=kth_lower,t_r_avg_l=kth_upper,rank_test=False)
        #get relaxation times for non-zero eigenvalues
        eigv_t=np.real(eigv_t)
        eigv_t_nz=np.array([e for e in eigv_t if e>cutoff_zero_eigv])
        tau_t=relax_times(zeta=zeta,b=b,k=k,T=T,eigv_lambda=eigv_t_nz)
...
#V2: pre-averaged hydr. interaction, by calculating <1/r> between beads n,m from simulation
#Version for RPY tensor
def get_conn_mat_eigv_hydro2_rpy(time_sel,obs_top,part_pos,edges_centre,edges_linker,box_size, eta, a,t_r_avg_f,t_r_avg_l,rank_test=True):
    #time_sel: time point to evaluate
    #obs_top: observed topologies for all time steps
    #part_pos: particle positions for all time steps
    #edges_centre: get vertices with this number of edges
    ...
    #pre-average Oseen 
    H_t=compute_pre_matrix_vec_rpy_reg(positions=centre_particle_coords_all_t, box_size=box_size,eta=eta, a=a)
