...
#Version 2: updated to include angle evaluation of CG surface

#Version 3: updated to include angle evaluation of conrad x-motif
...
#get angles between vectors defined by two particles defined by their id, for instace betwee surface sections
def conf_angles_group_id_1(pos,traj,particle_ids):
    angles_sections=[]
    ...
    for i in range(len(pos)):#iterate time points
        ...
        #vectors at given time: 
        angles_sections_t=[]
        for n in range(len(particle_ids)):#iterate arms
 
            pos_point_1=int(particle_ids[n][0])
            pos_base_1=int(particle_ids[n][1])
            pos_point_2=int(particle_ids[n][2])
            pos_base_2=int(particle_ids[n][3])
            
            vec_1=pos[i][pos_point_1]-pos[i][pos_base_1]
            vec_2=pos[i][pos_point_2]-pos[i][pos_base_2]
            #print(vec_1,vec_2,angle_1(vec_1,vec_2), np.dot(vec_1/np.linalg.norm(vec_1),vec_2/np.linalg.norm(vec_2)))
            angles_sections_t.append(angle_1(vec_1,vec_2))

        angles_sections.append(angles_sections_t)
    return np.asarray(angles_sections)
...
#get angle distributions for surface angles, based on particle ids, obtain full distribution before hist application 
def get_training_angle_distr_surf_1(folder,name,particle_ids,t_skip,t_lim,t_step,return_dict):
 
    #angle between surface sections
    #angle_surf_distribution_all=[]
    ...
    #angle between surface sections
    angle_surf_sections=conf_angles_group_id_1(pos=part_positions[t_skip:t_lim:t_step],traj=traj,particle_ids=particle_ids)

    key_name=name  #+".h5"
    return_dict[key_name]=[angle_surf_sections]

##########
#Functions for getting fitness score based on angle distributions
##########
...
###    
#for surface with sequence 2x ss fill + 4x ss int +2x ss fill
###

#return a fitness score based on surf_angle_distr
def eval_sim_1_1_ss_surf(folder,names_sim_repeat,t_skip,t_lim,t_step,angle_surf_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated

    #ids for surface with 1 rep unit and total length of 8 particles
    particle_ids=[[0,1,1,2],[1,2,2,3],[2,3,3,4],[3,4,4,5],[4,5,5,6],[5,6,6,7]]
    #set up parallel evaluation of angles
    f_eval_calls=[] #list of functions for evaluation in parallel
    input_params=[] #list of lists of parameters to use in each evaluation
    for j in range(len(names_sim_repeat)):
        #folder,name,arms_names_1,arms_names_2,arms_names_3,arms_names_4,t_skip,t_lim,t_step,return_dict
        input_params_e=[folder,names_sim_repeat[j],particle_ids,t_skip,t_lim,t_step]
        
        f_eval_calls.append(get_training_angle_distr_surf_1)
        input_params.append(input_params_e)
    ...
    for s in range(len(names_sim_repeat)):
        ...
        values_angle_surf_e=eval_dict[names_sim_repeat[s]][0].flatten()

        values_angle_surf.append(values_angle_surf_e) 

    values_angle_surf=np.asarray(values_angle_surf).flatten()
    
    #get value of ground truth at parameter set evaluated
    angle_surf_distr=np.histogram(values_angle_surf*180/np.pi,bins=np.arange(0,190,5),density=True)[0]
    
    angle_surf_distr=np.asarray(angle_surf_distr)    
    
    #calculate fitness score
    gt_value=gt_1_1(angle_surf_distr,angle_surf_distr_target)
...
#return a fitness score based on base_angle_distr,arm_axis_angle_distr,oppa_angle_distr,link_angle_distr
def eval_sim_c1_4_ba_aa_oa_la(folder,names_sim_repeat,t_skip,t_lim,t_step,base_angle_distr_target,arm_axis_angle_distr_target,oppa_angle_distr_target,link_angle_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated
    ...
    #names base angle 
    #get names of particles for vectors for angles, first entry: name of particle further away from centre
    arms_names_1=[["x_arm_a_c1","x_base_arm_a_c1"],["x_arm_b_c1","x_base_arm_b_c1"],["x_arm_c_c1","x_base_arm_c_c1"],["x_arm_d_c1","x_base_arm_d_c1"]]

    #names link angle 
    #each entry contains name of tip, then base of first vector and then tip and base of second vector, then repeat for next arm
    arms_names_2=[["x_link_a_c1","x_arm_a_c1","x_arm_a_c1","x_base_arm_a_c1"],["x_link_b_c1","x_arm_b_c1","x_arm_b_c1","x_base_arm_b_c1"],["x_link_c_c1","x_arm_c_c1","x_arm_c_c1","x_base_arm_c_c1"],["x_link_d_c1","x_arm_d_c1","x_arm_d_c1","x_base_arm_d_c1"]]

    #names arm axis angle 
    #each entry contains name of tip, then base of first vector and then tip and base of second vector, then repeat for next arm
    arms_names_3=[["x_arm_a_c1","x_base_arm_a_c1","x_base_arm_a_c1","x_centre_c1"],["x_arm_b_c1","x_base_arm_b_c1","x_base_arm_b_c1","x_centre_c1"],["x_arm_c_c1","x_base_arm_c_c1","x_base_arm_c_c1","x_centre_c1"],["x_arm_d_c1","x_base_arm_d_c1","x_base_arm_d_c1","x_centre_c1"]]

    #names opposing arms (identical to base angle)
    arms_names_4=[["x_arm_a_c1","x_base_arm_a_c1"],["x_arm_b_c1","x_base_arm_b_c1"],["x_arm_c_c1","x_base_arm_c_c1"],["x_arm_d_c1","x_base_arm_d_c1"]]
    ...
    return gt_value
#evaluate n sims in parallel, get angle distributions and apply fitnes function
#return a fitness score based on arm axis angle_distr,base_angle_distr,oppa_angle_distr
def eval_sim_c1_3_aa_ba_oa(folder,names_sim_repeat,t_skip,t_lim,t_step,arm_axis_angle_distr_target,base_angle_distr_target,oppa_angle_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated
    ...
    input_params=[] #list of lists of parameters to use in each evaluation
    for j in range(len(names_sim_repeat)):
        #folder,name,arms_names_c1,arms_names_2,arms_names_3,arms_names_4,t_skip,t_lim,t_step,return_dict
        input_params_e=[folder,names_sim_repeat[j],arms_names_1,arms_names_2,arms_names_3,arms_names_4,t_skip,t_lim,t_step]
    ...
    #calculate fitness score
    gt_value=gt_1_3(arm_axis_angle_distr,base_angle_distr,oppa_angle_distr,      arm_axis_angle_distr_target,base_angle_distr_target,oppa_angle_distr_target)
    
    return gt_value
#evaluate n sims in parallel, get angle distributions and apply fitnes function
#return a fitness score based on base_angle_distr,oppa_angle_distr
def eval_sim_c1_2_ba_oa(folder,names_sim_repeat,t_skip,t_lim,t_step,base_angle_distr_target,oppa_angle_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated
...
#evaluate n sims in parallel, get angle distributions and apply fitnes function
#return a fitness score based on link_angle_distr
def eval_sim_c1_1_la(folder,names_sim_repeat,t_skip,t_lim,t_step,link_angle_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated
...
#evaluate n sims in parallel, get angle distributions and apply fitnes function
#return a fitness score based on link_angle_distr
def eval_sim_c1_1_aa(folder,names_sim_repeat,t_skip,t_lim,t_step,arm_axis_angle_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated
...
#evaluate n sims in parallel, get angle distributions and apply fitnes function
#return a fitness score based on link_HP_angle_distr
def eval_sim_c1_1_hp(folder,names_sim_repeat,t_skip,t_lim,t_step,link_HP_angle_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated
    ...
    #names link HP angle 
    #each entry contains name of tip, then base of first vector and then tip and base of second vector, then repeat for next arm
    arms_names_2=[["x_link_hp_a_c1","x_arm_a_c1","x_arm_a_c1","x_base_arm_a_c1"],["x_link_hp_b_c1","x_arm_b_c1","x_arm_b_c1","x_base_arm_b_c1"],["x_link_hp_c_c1","x_arm_c_c1","x_arm_c_c1","x_base_arm_c_c1"],["x_link_hp_d_c1","x_arm_d_c1","x_arm_d_c1","x_base_arm_d_c1"]]
...
#evaluate n sims in parallel, get angle distributions and apply fitnes function
#return a fitness score based on angle_ds_fused_link_distr
def eval_sim_c1_1_bm(folder,names_sim_repeat,t_skip,t_lim,t_step,angle_ds_fused_link_distr_target):
    #names_sim_repeat paths to n sims that are to be evaluated

    arms_names_2_fused=["x_arm_a_c1","x_link_fused_a_c1","x_link_fused_c_c1","x_arm_c_c1"]
...
### 
#Return angle distribution for evaluation
###
...
#evaluate n sims in parallel, get angle distributions
#for surface with sequence 2x ss fill + 4x ss int +2x ss fill
#values_angle_surf, arm_axis_angle_distr
def cg_angle_distr_1_1_ss_surf(folder,names_sim_repeat,t_skip,t_lim,t_step):
    #names_sim_repeat paths to n sims that are to be evaluated
    ...
    return values_angle_surf, angle_surf_distr
 

#Conrad X-motif
#evaluate n sims in parallel, get angle distributions 
#return base_angle_distr,arm_axis_angle_distr,oppa_angle_distr,link_angle_distr,    values_angle_base,values_angle_link,values_angle_arm_axis,values_angle_oppa
def cg_angle_distr_c1_4_ba_aa_oa_la(folder,names_sim_repeat,t_skip,t_lim,t_step):
    #names_sim_repeat paths to n sims that are to be evaluated
...
#evaluate n sims in parallel, get angle distributions 
#return link_angle_distr,    values_angle_link
def cg_angle_distr_c1_1_hp(folder,names_sim_repeat,t_skip,t_lim,t_step):
    #names_sim_repeat paths to n sims that are to be evaluated
...
#evaluate n sims in parallel, get angle distributions 
#angle_ds_fused_link_distr, values_angle_ds_fused_link
def cg_angle_distr_c1_1_bm(folder,names_sim_repeat,t_skip,t_lim,t_step):
    #names_sim_repeat paths to n sims that are to be evaluated
